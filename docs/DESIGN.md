...
| 初期実装（ルールベース） | ①直近の閲覧・検索履歴から関連タグ＋都道府県の出現頻度を集計（時間減衰係数つき）②未閲覧銘柄をタグ一致度でスコアリング ③上位 N 件を返す。単一の集計 SQL＋スコア計算の純関数で実装（スコア重みは定数化） |
...
| `import-sakenowa.ts` | areas / brands / breweries / rankings / flavor-charts / brand-flavor-tags を取得し、**さけのわ brandId を自然キーに冪等 upsert**。フレーバーチャート6軸をしきい値（定数）で味タグ（淡麗/濃醇/華やか等）へ機械変換 |
...
3. 履歴が十分ある場合:
   a. 直近の ViewHistory（閲覧銘柄のタグ・都道府県）と SearchHistory（filters 内のタグ・都道府県）を集計し、
      時間減衰係数を掛けた頻度スコアを作る（1 本の集計 SQL）。
   b. 未閲覧銘柄を対象に、タグ一致度 × 頻度スコアで並べ替え、上位 `limit` 件を返す
      （reason: "よく見ている『辛口』『山口県』から" のような根拠タグ付き）。
...
| RAG の検索精度・ヒアリング品質 | PoC（FEASIBILITY R3/R4: 銘柄 50 件×質問 10 パターン）。結果次第でプロンプト・retriever の重みを調整 |
