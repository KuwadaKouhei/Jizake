...
| 2 | 検索 | `src/app/search/`（URL⇔条件の純関数は `src/lib/search-query/`。T15 昇格） |
...
│  │  │  # 注1: 検索クエリ searchSakes は SakeSummary・タグ一括取得を県別一覧と共有するため
│  │  │  #      _lib/search-sakes.ts を作らず src/lib/db/queries/sakes.ts に集約（DIR-6。T07）
│  │  │  # 注2: URL⇔検索条件の純関数（旧 _lib/build-search-query）は 3 機能（検索・履歴・
│  │  │  #      チャット fallback）で使うため src/lib/search-query/ へ昇格（DIR-11。T15）
│  │  ├─ sake/
│  │  │  └─ [id]/                      # ─ カタログ: 詳細 ─
│  │  │     ├─ page.tsx
...
│  │  ├─ pagination/                   # ─ 一覧・検索共通のページ番号処理（純関数） ─
│  │  │  ├─ pagination.ts              # parsePageParam / totalPageCount（T07 で県別 _lib から昇格）
│  │  │  └─ pagination.test.ts
│  │  ├─ search-query/                 # ─ URL⇔検索条件の純関数（検索・履歴・チャットfallback共用） ─
│  │  │  ├─ index.ts                   # SearchCriteria/build/toSearchQueryString/isEmpty/sanitize（T15 昇格・DIR-11）
│  │  │  └─ index.test.ts
...
| `scripts/` | データ投入バッチ・使い捨て評価ハーネス（ローカル tsx 実行） | `src/lib/db`・`src/lib/ai`・`src/lib/rag`（PoC 評価で retriever を呼ぶ。T13）を import してよい。`src/app` は import 禁止 |
...
- `scripts/` は `src/lib/db`・`src/lib/ai`・`src/lib/rag`（PoC 評価で retriever を呼ぶ。T13）・`src/lib/constants`・`scripts/lib` のみ import 可。
  `src/app`・`src/components` は import 禁止（バッチが UI に依存しない）。
- 逆流や横断の誘惑が生じたら、それは共有クエリ（`lib/db/queries`）への抽出シグナルであり、
  機能同士を直接つなぐ理由にはしない。
- **検索条件の URL 表現は `src/lib/search-query/` に昇格済み（決定 DIR-11 の予告どおり。T15）**:
  `SearchCriteria` 型・`toSearchQueryString`・`isEmptyCriteria`・`sanitizeCriteria`・`buildSearchCriteria`・
  `RawSearchParams` は当初 T09 で「履歴（`src/app/history/_lib`）→検索（`src/app/search/_lib`）の一方向参照」
  として許容していたが、**検索(T07)・履歴(T09)・チャットのフォールバック導線(T15 `api/chat/_lib/fallback-search.ts`)**
  の 3 機能が参照する Rule of Three の昇格トリガに到達したため、責務名ディレクトリ `src/lib/search-query/`
  （URL⇔条件の唯一の情報源）へ移した。これで機能固有 `_lib` 同士の参照が消え、循環もない。pagination
  （DIR-10）と同型の昇格。
...
| DIR-10 | ページ番号処理（parsePageParam / totalPageCount）の置き場 | `src/lib/pagination/`（責務名ディレクトリ）へ昇格 | 県別 `_lib/pagination.ts` に据え置き、検索から相対 import | T06（県別一覧）と T07（検索）の 2 機能が同じページ番号処理を必要とする。検索の `_lib` から県別の `_lib/pagination.ts` を import すると §5.2「機能ロジック同士は横に依存しない」に反する。Rule of Three は「3 箇所目まではコピー許容」だが、コピーではなくパス依存が問題のため、2 箇所目で責務名ディレクトリへ昇格し両機能が共有領域を参照する形にした（純関数・DB 非依存で降格リスクも低い） |
| DIR-11 | 履歴の再検索リンクが使う検索 URL 表現の参照方法 | 履歴 `_lib` から検索 `_lib`（`SearchCriteria`・`toSearchQueryString`・`isEmptyCriteria`）を一方向 import | `src/lib/search-query/` へ即昇格／履歴側で URL 組み立てを再実装 | 検索が「URL⇔検索条件」の唯一の情報源であり、履歴の再検索・記録スキップ判定はその表現を再利用するのが正しい（再実装は二重定義でズレる）。DIR-10 の pagination と違い `SearchCriteria` は検索の中核型で、丸ごと `src/lib` へ移すと検索側が破壊的に変わる。循環のない一方向参照（検索は履歴を知らない）に留め、3 機能目が現れたら責務名ディレクトリへ昇格する（§5.2 例外として明記） |
