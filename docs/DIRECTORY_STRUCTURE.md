...
- 逆流や横断の誘惑が生じたら、それは共有クエリ（`lib/db/queries`）への抽出シグナルであり、
  機能同士を直接つなぐ理由にはしない。
- **例外（T09）**: 履歴（`src/app/history/_lib`）は検索（`src/app/search/_lib/build-search-query.ts`）の
  `SearchCriteria` 型・`toSearchQueryString`・`isEmptyCriteria` を**一方向で参照**する。履歴の再検索リンクは
  本質的に「検索の URL 表現」そのものであり、検索が URL⇔条件の唯一の情報源だからである（決定 DIR-11）。
  検索は履歴を知らず循環はない。pagination（DIR-10）のように `src/lib` へ昇格しないのは、`SearchCriteria` が
  検索機能の中核型で丸ごと共有領域へ移すと検索側の破壊的変更になるため。3 機能目が同じ表現を必要としたら
  そのとき責務名ディレクトリ（例: `src/lib/search-query/`）へ昇格する。
