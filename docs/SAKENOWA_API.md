...
| エンドポイント | 内容 | 件数（2026-07-04 実測） |
|---|---|---|
| `/areas` | 地域（都道府県）マスタ | 48（1〜47 + 0=その他） |
| `/brands` | 銘柄マスタ | 3,253 |
| `/breweries` | 蔵元マスタ | 1,749 |
| `/rankings` | 総合・地域別人気ランキング（月次） | 総合100位 + 地域別各1〜50位 |
| `/flavor-charts` | フレーバー6軸（銘柄別） | 1,341（**全銘柄にはない**） |
| `/flavor-tags` | フレーバータグマスタ | 141 |
| `/brand-flavor-tags` | 銘柄→タグID群 | 2,970（うち空配列 829） |
...
- **空文字名の蔵元が 48 件存在する**（2026-07-04 実測。id 784〜830 が各都道府県に
  1 件ずつ＋id 1147。「蔵元不明」プレースホルダとみられ、262 銘柄がこれらを参照）。
  → 蔵元名を表示できないためインポートではスキップし、件数をログ出力する。
- **同一（name, areaId）の重複蔵元が 43 組存在する**（例: 北島酒造 id 499 / 908）。
  `UNIQUE (name, prefecture_code)` に抵触するため、インポートでは最初の 1 件に
  統合し、重複側を参照する銘柄は統合先の蔵元へ付け替える。
...
```
- 141 種（2026-07-04 実測）。「酸味」「辛口」「旨味」「フルーティ」「スッキリ」「甘味」「コク」など、
...
| 味タグの供給 | ✅ flavor-tags 141 種 + brand-flavor-tags。空配列あり |
