...
### T25: お気に入り機能（ログインユーザー）

| 項目 | 内容 |
|---|---|
| 概要 | ログインユーザーが日本酒をお気に入り登録/解除でき、一覧で見返せる（ユーザー指示 2026-07-05・FR-10） |
| 主な作業内容 | ① favorites テーブル（複合 PK・index・RLS: 本人 SELECT のみ。マイグレーション 0004/0005）② queries/favorites.ts（is/toggle/list・PGlite テスト）③ toggleFavorite / readFavoriteState（Server Action・user_id はセッションから）④ FavoriteButton（詳細ページの自己完結・動的アイランド。楽観的更新・未ログインはログイン誘導）⑤ /favorites 一覧ページ（保護ルート）⑥ ヘッダー・下部タブに導線・PROTECTED_PREFIXES に追加 |
| 受け入れ条件 | FR-10 の全受け入れ条件 |
| 依存タスク | T05, T08 |
| ブランチ | `feature/T25-favorites` |
| 状態 | 完了 |
...
| FR-10 | 詳細でお気に入り追加/解除・状態表示／未ログインは誘導／一覧表示・詳細遷移／本人のみ参照 | T25 |
