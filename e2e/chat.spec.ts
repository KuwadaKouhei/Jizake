...
import { hasChatLlm } from "./_support/env";
 ...
 * - フルフロー（要 ANTHROPIC_API_KEY）: 実 LLM（Claude API 直接接続）への 1 メッセージ送信
 *   →ストリーミング応答。実キーが要るため、キーがあるときだけ実行する（test.skip(!hasChatLlm)）。
 */
...
test.describe("導線③ チャット 1 往復（フルフロー・要 ANTHROPIC_API_KEY）", () => {
  test.skip(
    !hasChatLlm,
    "ANTHROPIC_API_KEY 未設定。実 LLM への 1 往復はキーがある環境でのみ実行する。",
  );
